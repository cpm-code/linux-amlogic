		...
		if (type == PTS_TYPE_VIDEO) {
			...
			*page_offset = offset - pts_table[PTS_TYPE_VIDEO].buf_start;
			...
			do {
				...
				page1 = READ_AIU_REG(AIU_MEM_AIFIFO_BUF_WRAP_COUNT) & 0xffff;
				offset = READ_AIU_REG(AIU_MEM_AIFIFO_MAN_RP);
				page2 = READ_AIU_REG(AIU_MEM_AIFIFO_BUF_WRAP_COUNT) & 0xffff;
...
int calculation_stream_ext_delayed_ms(u8 type)
{
	...
	if (extdelay_ms < 0) extdelay_ms = 0;

	return extdelay_ms * 1000 / HZ;
}
EXPORT_SYMBOL(calculation_stream_ext_delayed_ms);

static inline void pts_checkin_offset_calc_cached(u32 offset, u32 val, struct pts_table_s *ptable)
{
...
static int pts_checkin_offset_inline(u8 type, u32 offset, u32 val, u64 us64)
{
	...
	if (type >= PTS_TYPE_MAX) return -EINVAL;

	ptable = &pts_table[type];

	spin_lock_irqsave(&lock, flags);

	if (likely((ptable->status == PTS_RUNNING) || (ptable->status == PTS_LOADING)))
	{
		...
		if (type == PTS_TYPE_AUDIO && ptable->first_checkin_pts == -1)
		{
			...
			pr_info("[pts_kpi] first check in apts <0x%x:0x%x(0x%llx)> ok!\n", offset, val, us64);
		}

		if (tsync_get_debug_pts_checkin())
		{
			if (tsync_get_debug_vpts() && (type == PTS_TYPE_VIDEO)) {
				pr_info("check in vpts <0x%x:0x%x>,", offset, val);
		...
		if (list_empty(&ptable->free_list))
			rec = list_entry(ptable->valid_list.next, struct pts_rec_s, list);
		else
			rec = list_entry(ptable->free_list.next, struct pts_rec_s, list);

		if (!list_empty(&ptable->valid_list))
		{
			rec_prev = list_entry(ptable->valid_list.prev, struct pts_rec_s, list);
			if (rec_prev->offset == ptable->last_checkin_offset)
			{
				if (offset > ptable->last_checkin_offset)
					rec_prev->size = offset - ptable->last_checkin_offset;
				else
					rec_prev->size = 0;
			}
		}
		...
		if ((ptable->last_checkin_offset > 0) &&
			...
			rec->size = offset - ptable->last_checkin_offset;
		else
...
int pts_checkin_wrptr(u8 type, u32 ptr, u32 val)
{
	...
	if (tsync_get_new_arch()) return -EINVAL;
...
int pts_checkin_wrptr_pts33(u8 type, u32 ptr, u64 pts_val)
{
	...
	return pts_checkin_offset_inline(type, pts_table[type].buf_size * page_no + offset, (u32)pts_val, us);
}
...
static int pts_pick_by_offset_inline_locked(u8 type, u32 offset, u32 *val, u32 pts_margin, u64 *us64)
{
	struct pts_table_s *ptable;
	int lookup_threshold;
	...
	if (likely(ptable->status == PTS_RUNNING))
	{
		...
		if (OFFSET_LATER(offset, p->offset))
		{
			...
			list_for_each_entry_continue(p, &ptable->valid_list, list)
			{
				...
				if (OFFSET_LATER(p->offset, offset)) break;
		...
		if ((p2) && (OFFSET_DIFF(offset, p2->offset) < lookup_threshold)) {
			...
			if (tsync_get_debug_pts_checkout())
			{
				...
				if (tsync_get_debug_apts() && (type == PTS_TYPE_AUDIO)) {
					pr_info("apts look up offset<0x%x> -->", offset);
					pr_info("<0x%x:0x%x>, look_cnt = %d\n", p2->offset, p2->val, look_cnt);
				}
			}
			*val = p2->val;
...
static int pts_lookup_offset_inline(u8 type, u32 offset, u32 *val, u32 *frame_size, u32 pts_margin, u64 *us64)
{
	unsigned long flags;
	int res;
	...
	else if ((timestamp_firstapts_get() == 0) &&
	         (res == 0) &&
           ((*val) != 0) &&
	         (type == PTS_TYPE_AUDIO))
	{
...
static int pts_pick_by_offset_inline(u8 type, u32 offset, u32 *val, u32 pts_margin, u64 *us64)
{
	unsigned long flags;
	int res;
	...
	res = pts_pick_by_offset_inline_locked(type, offset, val, pts_margin, us64);
...
int pts_lookup_offset(u8 type, u32 offset, u32 *val, u32 *frame_size, u32 pts_margin)
{
	u64 pts_us;

	return pts_lookup_offset_inline(type, offset, val, frame_size, pts_margin, &pts_us);
}
EXPORT_SYMBOL(pts_lookup_offset);

int pts_lookup_offset_us64(u8 type, u32 offset, u32 *val, u32 *frame_size, u32 pts_margin, u64 *us64)
{
	return pts_lookup_offset_inline(type, offset, val, frame_size, pts_margin, us64);
}
EXPORT_SYMBOL(pts_lookup_offset_us64);

int pts_pickout_offset_us64(u8 type, u32 offset, u32 *val, u32 pts_margin, u64 *us64)
{
...
int pts_get_rec_num(u8 type, u32 val)
{
	...
	if (ptable->status != PTS_RUNNING) goto out;

	if (list_empty(&ptable->valid_list)) goto out;
	...
	if (OFFSET_LATER(val, p->offset))
	{
		list_for_each_entry_continue(p, &ptable->valid_list, list)
		{
			if (OFFSET_LATER(p->offset, val)) break;
		}
	}
	...
	if (likely(ptable->status == PTS_IDLE))
	{
		...
		if (has_hevc_vdec() && (type == PTS_TYPE_HEVC)) {
			...
			if (!tsync_get_new_arch()) {
				...
				ptable->buf_size = READ_PARSER_REG(PARSER_VIDEO_END_PTR) - ptable->buf_start + 8;
			}
#else
			if (!tsync_get_new_arch()) {
				ptable->buf_start = READ_VREG(HEVC_STREAM_START_ADDR);
				ptable->buf_size = READ_VREG(HEVC_STREAM_END_ADDR) - ptable->buf_start;
			}
#endif
			...
			if (type == PTS_TYPE_VIDEO)
			{
				...
				if (!tsync_get_new_arch()) {
					...
					ptable->buf_size = READ_VREG(VLD_MEM_VIFIFO_END_PTR) - ptable->buf_start + 8;
				}
#endif
			...
			else if (type == PTS_TYPE_AUDIO)
			{
				if (!tsync_get_new_arch())
				{
					ptable->buf_start = READ_AIU_REG(AIU_MEM_AIFIFO_START_PTR);
					ptable->buf_size = READ_AIU_REG(AIU_MEM_AIFIFO_END_PTR) - ptable->buf_start + 8;
				}
