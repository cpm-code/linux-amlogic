...
/* core2: graphic priority 	-- linked to 2A?    has a lut	OSD 1080P (or HDR etc.) */
...
#define FLAG_FORCE_CVM				0x01		/* Force CVM (RPU Processing - for LLDV, or TV-LED (Graphic Priority) modes in Dolby (Core1)) */
#define FLAG_BYPASS_CVM				0x02		/* Bypass the CVM (RPU Processing for LLDV, or TV-LED (Graphic Priority) modes in Dolby (Core1)) */		
...
#define FLAG_DISABLE_COMPOSER		0x100		/* Disable BL and EL - no output ? */
#define FLAG_BYPASS_CSC				0x200		/* Bypass the Colour Space Converter in Dolby (Core1) */
#define FLAG_CHECK_ES_PTS			0x400		/* Check the EL layer Elementary Strem - Presentation Time Stamp  */
#define FLAG_DISABE_CORE_SETTING	0x800		/* Dont set the cores - why? */
#define FLAG_DISABLE_DMA_UPDATE		0x1000		/* ? */
#define FLAG_DISABLE_DOVI_OUT		0x2000		/* Foce DoVi Sink (TV) to behave as not support DoVi */
...
/*
1	00001 	Core 1 (DoVi) 	Reset
2	00010	Core 2 (OSD) 	Reset
4	00100	Core 3 (RPU) 	Reset
8	01000	Core 1 (DoVi)  	Lut
16	10000	Core 2 (OSD)  	Lut	
31	11111	All Reset
*/	
		 ...
		 bool reset)
{
	...
	if (bl_enable) {
		...
		if (get_vpu_mem_pd_vmod(VPU_DOLBY1A) == VPU_MEM_POWER_DOWN || get_dv_mem_power_flag(VPU_DOLBY1A) == VPU_MEM_POWER_DOWN)
			dv_mem_power_on(VPU_DOLBY1A);

		if (get_vpu_mem_pd_vmod(VPU_PRIME_DOLBY_RAM) == VPU_MEM_POWER_DOWN || get_dv_mem_power_flag(VPU_PRIME_DOLBY_RAM) == VPU_MEM_POWER_DOWN)
		 ...
		 int lut_endian)
{
	...
	VSYNC_WR_DV_REG(DOLBY_CORE2A_SWAP_CTRL1,
					((hsize + g_htotal_add) << 16) | (vsize
													  + ((g_vtiming & 0xff000000) ? ((g_vtiming >> 24) & 0xff) : g_vtotal_add)
													  + ((g_vtiming & 0xff0000) ? ((g_vtiming >> 16) & 0xff) : g_vsize_add)));

	VSYNC_WR_DV_REG(DOLBY_CORE2A_SWAP_CTRL2,
					(hsize << 16) | (vsize + ((g_vtiming & 0xff0000) ?((g_vtiming >> 16) & 0xff) : g_vsize_add)));

	VSYNC_WR_DV_REG(DOLBY_CORE2A_SWAP_CTRL3,
					(g_hwidth << 16) | ((g_vtiming & 0xff00) ? ((g_vtiming >> 8) & 0xff) : g_vwidth));

	VSYNC_WR_DV_REG(DOLBY_CORE2A_SWAP_CTRL4,
					(g_hpotch << 16) | ((g_vtiming & 0xff) ? (g_vtiming & 0xff) : g_vpotch));
		 ...
		 int frame_cnt, int debug_flag)
{
	...
	if ((debug_flag & 0x10) && dump_enable && !is_graphics_output_off()) {
		...
		for (i = DOLBY_CORE2A_CLKGATE_CTRL; i <= DOLBY_CORE2A_DMA_PORT; i++)
			pr_info("[0x%4x] = 0x%x\n", i, READ_VPP_DV_REG(i));
			
		pr_info("core2 real reg\n");
			
		for (i = DOLBY_CORE2A_REG_START; i <= DOLBY_CORE2A_REG_START + 5; i++)
			pr_info("[0x%4x] = 0x%x\n", i, READ_VPP_DV_REG(i));
	}
	...
	if ((debug_flag & 0x10) && dump_enable) {
		...
		for (i = 0; i < 26; i++)
			pr_info("%08x\n", p[i]);
		pr_info("core3 swap\n");
		for (i = DOLBY_CORE3_CLKGATE_CTRL; i <= DOLBY_CORE3_OUTPUT_CSC_CRC; i++)
			pr_info("[0x%4x] = 0x%x\n", i, READ_VPP_DV_REG(i));
		pr_info("core3 real reg\n");
		for (i = DOLBY_CORE3_REG_START; i <= DOLBY_CORE3_REG_START + 67; i++)
			pr_info("[0x%4x] = 0x%x\n", i, READ_VPP_DV_REG(i));
	}
		...
		(enum signal_format_enum src_format, struct vframe_s *vf)
{
	...
	if (src_format == FORMAT_DOVI || src_format == FORMAT_DOVI_LL) {
		dolby_vision_src_format = 3; /* DOVI */
	} else {
		if (vf) {
			if (vf_is_hdr10_plus(vf))
				dolby_vision_src_format = 2; /* HDR+ */
			else if (vf_is_hdr10(vf))
				dolby_vision_src_format = 1; /* HDR */
			else if (vf_is_hlg(vf))
				dolby_vision_src_format = 5; /* HLG */
			else if (is_mvc_frame(vf))
				dolby_vision_src_format = 7; /* MVC */
			else
				dolby_vision_src_format = 6; /* SDR */
		}
	}
...
{	
	...
	/* If MVC (Multi View Coding?) - Then cannot use DoVi */
	...
	/* If CUVA - (China HDR Format?) - Then cannot use DoVi */
	if (src_format == FORMAT_CUVA) {
		if (dolby_vision_mode != DOLBY_VISION_OUTPUT_MODE_BYPASS) {
			if (debug_dolby & 2)
				pr_dolby_dbg("cuva, dovi output -> BYPASS\n");
			*mode = DOLBY_VISION_OUTPUT_MODE_BYPASS;
			mode_change = 1;
		} else {
			mode_change = 0;
	...
	/* Follow the TV - if can do DoVi then let it convert to DoVi ? */
	if (dolby_vision_policy == DOLBY_VISION_FOLLOW_SINK) {
		...
		/* HLG - cannot convert to DoVi - so bypass */
		/* HDR10Plus and policy not to convert to DoVi - then bypass */
		if (src_format == FORMAT_HLG || (src_format == FORMAT_HDR10PLUS && !(dolby_vision_hdr10_policy & HDRP_BY_DV))) {
			...
			/*****************************************************/
			/* If we get there then sending down the DoVi Tunnel */
			/*****************************************************/
		...
		/* Bypass if HDR is on (and format not DoVi) */
...
int dolby_vision_check_mvc(struct vframe_s *vf)
{
	...
	if (is_mvc_frame(vf) && dolby_vision_on) {
		...
		if (dolby_vision_policy_process(&mode, FORMAT_MVC)) {
			if (mode != DOLBY_VISION_OUTPUT_MODE_BYPASS && dolby_vision_mode == DOLBY_VISION_OUTPUT_MODE_BYPASS)
				dolby_vision_wait_on = true;
...
static int dolby_vision_vf_check(struct vframe_s *vf)
{
	...
	for (i = 0; i < 16; i++) {
		if (dv_vf[i][0] == vf) {
			if (debug_dolby & 2) {
				if (dv_vf[i][1])
					pr_dolby_dbg("=== bl(%p-%lld) with el(%p-%lld) toggled ===\n", vf, vf->pts_us64, dv_vf[i][1], dv_vf[i][1]->pts_us64);
				else
						   ...
						   char *comp_buf)
{
	...
	if (!aux_buf || aux_size == 0 || !fmt || !md_buf || !comp_buf || !total_comp_size || !total_md_size || !ret_flags)
		return 1;
	...
	while (p < aux_buf + aux_size - 8) {
		...
		if (type == DV_SEI || /* hevc t35 sei */
			...
			*total_md_size = 0;
			 ...
			 * after parsing dolby type, breaking the circulation directly */
			break;
		}
		p += size;
	}
	...
	if (atsc_sei && *src_format != FORMAT_DOVI) {
		...
		while (p < aux_buf + aux_size - 8) {
			...
			if (type == 0x02000000) {
				...
				if (nal_type == PREFIX_SEI_NUT_NAL || nal_type == SUFFIX_SEI_NUT_NAL) {
		...
		if (len_2094_sei > 0) {
			...
			*src_format = FORMAT_DOVI;
			...
			if (!metadata_parser) {
				...
				if (p_funcs_stb->metadata_parser_reset(reset_flag | metadata_parser_reset_flag) == 0)
					metadata_parser_reset_flag = 0;
	...
	if (*total_md_size) {
		...
		if ((debug_dolby & 1) && vf)
			pr_dolby_dbg("meta(%d), pts(%lld) -> md(%d), comp(%d)\n", size, vf->pts_us64, *total_md_size, *total_comp_size);
		 ...
		 enum signal_format_enum src_format)
{
	...
	if (vinfo->vout_device->dv_info && vinfo->vout_device->dv_info->sup_backlight_control && (setting->ext_md.avail_level_mask & EXT_MD_LEVEL_2)) {
		 ...
		 const struct vinfo_s *vinfo, struct vframe_s *vf)
{
	...
	if (dst_format == FORMAT_HDR10) {
		...
		if (src_format != FORMAT_HDR10PLUS) {
			...
			if (hdr10_data.primaries[0][0] != ((p_hdr->primaries_x_0_msb << 8) | p_hdr->primaries_x_0_lsb))
				flag = true;
			
			hdr10_data.primaries[0][0] = (p_hdr->primaries_x_0_msb << 8) | p_hdr->primaries_x_0_lsb;

			if (hdr10_data.primaries[0][1] != ((p_hdr->primaries_y_0_msb << 8) | p_hdr->primaries_y_0_lsb))
				flag = true;
			
			hdr10_data.primaries[0][1] = (p_hdr->primaries_y_0_msb << 8) | p_hdr->primaries_y_0_lsb;

			if (hdr10_data.primaries[1][0] != ((p_hdr->primaries_x_1_msb << 8) | p_hdr->primaries_x_1_lsb))
				flag = true;
			
			hdr10_data.primaries[1][0] = (p_hdr->primaries_x_1_msb << 8) | p_hdr->primaries_x_1_lsb;

			if (hdr10_data.primaries[1][1] != ((p_hdr->primaries_y_1_msb << 8) | p_hdr->primaries_y_1_lsb))
				flag = true;
			
			hdr10_data.primaries[1][1] = (p_hdr->primaries_y_1_msb << 8) | p_hdr->primaries_y_1_lsb;
			
			if (hdr10_data.primaries[2][0] != ((p_hdr->primaries_x_2_msb << 8) | p_hdr->primaries_x_2_lsb))
				flag = true;
			
			hdr10_data.primaries[2][0] = (p_hdr->primaries_x_2_msb << 8) | p_hdr->primaries_x_2_lsb;

			if (hdr10_data.primaries[2][1] != ((p_hdr->primaries_y_2_msb << 8) | p_hdr->primaries_y_2_lsb))
				flag = true;
			...
			if (hdr10_data.primaries[0][0] != ((p_hdr->primaries_x_1_msb << 8) | p_hdr->primaries_x_1_lsb))
				flag = true;
			
			hdr10_data.primaries[0][0] = (p_hdr->primaries_x_1_msb << 8) | p_hdr->primaries_x_1_lsb;

			if (hdr10_data.primaries[0][1] != ((p_hdr->primaries_y_1_msb << 8) | p_hdr->primaries_y_1_lsb))
				flag = true;
			
			hdr10_data.primaries[0][1] = (p_hdr->primaries_y_1_msb << 8) | p_hdr->primaries_y_1_lsb;

			if (hdr10_data.primaries[1][0] != ((p_hdr->primaries_x_2_msb << 8) | p_hdr->primaries_x_2_lsb))
				flag = true;
			
			hdr10_data.primaries[1][0] = (p_hdr->primaries_x_2_msb << 8) | p_hdr->primaries_x_2_lsb;

			if (hdr10_data.primaries[1][1] != ((p_hdr->primaries_y_2_msb << 8) | p_hdr->primaries_y_2_lsb))
				flag = true;
			
			hdr10_data.primaries[1][1] = (p_hdr->primaries_y_2_msb << 8) | p_hdr->primaries_y_2_lsb;
			
			if (hdr10_data.primaries[2][0] != ((p_hdr->primaries_x_0_msb << 8) | p_hdr->primaries_x_0_lsb))
				flag = true;
			
			hdr10_data.primaries[2][0] = (p_hdr->primaries_x_0_msb << 8) | p_hdr->primaries_x_0_lsb;

			if (hdr10_data.primaries[2][1] != ((p_hdr->primaries_y_0_msb << 8) | p_hdr->primaries_y_0_lsb))
				flag = true;
			
			hdr10_data.primaries[2][1] = (p_hdr->primaries_y_0_msb << 8) | p_hdr->primaries_y_0_lsb;
		}

		if (hdr10_data.white_point[0] != ((p_hdr->white_point_x_msb << 8) | p_hdr->white_point_x_lsb))
			flag = true;
			
		hdr10_data.white_point[0] = (p_hdr->white_point_x_msb << 8) | p_hdr->white_point_x_lsb;

		if (hdr10_data.white_point[1] != ((p_hdr->white_point_y_msb << 8) | p_hdr->white_point_y_lsb))
			flag = true;
			
		hdr10_data.white_point[1] = (p_hdr->white_point_y_msb << 8) | p_hdr->white_point_y_lsb;

		if (hdr10_data.luminance[0] != ((p_hdr->max_display_mastering_lum_msb << 8) | p_hdr->max_display_mastering_lum_lsb))
			flag = true;
			
		hdr10_data.luminance[0] = (p_hdr->max_display_mastering_lum_msb << 8) | p_hdr->max_display_mastering_lum_lsb;

		if (hdr10_data.luminance[1] != ((p_hdr->min_display_mastering_lum_msb << 8) | p_hdr->min_display_mastering_lum_lsb))
			flag = true;
			
		hdr10_data.luminance[1] = (p_hdr->min_display_mastering_lum_msb << 8) | p_hdr->min_display_mastering_lum_lsb;

		if (hdr10_data.max_content != ((p_hdr->max_content_light_level_msb << 8) | p_hdr->max_content_light_level_lsb))
			flag = true;
			
		hdr10_data.max_content = (p_hdr->max_content_light_level_msb << 8) | p_hdr->max_content_light_level_lsb;

		if (hdr10_data.max_frame_average != ((p_hdr->max_frame_avg_light_level_msb << 8) | p_hdr->max_frame_avg_light_level_lsb))
			flag = true;
		...
		if (flag && debug_dolby & 8) {
			...
			for (i = 0; i < 3; i++)
				pr_dolby_dbg("\tprimaries[%1d] = %04x, %04x\n", i, hdr10_data.primaries[i][0], hdr10_data.primaries[i][1]);
			pr_dolby_dbg("\twhite_point = %04x, %04x\n", hdr10_data.white_point[0], hdr10_data.white_point[1]);
		...
		if (last_dst_format == FORMAT_HDR10) {
			...
			if (vinfo && vinfo->vout_device && vinfo->vout_device->fresh_tx_vsif_pkt) {
				if (vf && (is_hlg_frame(vf) || is_hdr10plus_frame(vf) || is_cuva_frame(vf))) {
		 ...
		 const struct vinfo_s *vinfo)
{
	...
	if (dst_format == FORMAT_DOVI) {
		...
		if (vinfo && vinfo->vout_device &&
			...
			if (dovi_ll_enable)
				vinfo->vout_device->fresh_tx_vsif_pkt
						(EOTF_T_DV_AHEAD, diagnostic_enable ? RGB_10_12BIT : YUV422_BIT12, &vsif, false);
			else
				vinfo->vout_device->fresh_tx_vsif_pkt
						(EOTF_T_DV_AHEAD, dolby_vision_target_mode == dovi_ll_enable ? YUV422_BIT12 : RGB_8BIT, &vsif, false);
		}
								...
								bool drop_flag)
{
	...
	if (vf && (vf->source_type == VFRAME_SOURCE_TYPE_OTHERS)) {
		...
		if (req.dv_enhance_exist && toggle_mode == 1 && dvel_provider && !strcmp(dvel_provider, "dveldec")) {
			
			el_vf = dvel_vf_get();
			
			if (el_vf && (el_vf->pts_us64 == vf->pts_us64 || !(dolby_vision_flags & FLAG_CHECK_ES_PTS))) {
	...
	/* check dst format */
	if (dolby_vision_mode == DOLBY_VISION_OUTPUT_MODE_IPT_TUNNEL || dolby_vision_mode == DOLBY_VISION_OUTPUT_MODE_IPT)
	...
	if (dolby_vision_graphic_max != 0) {
		...
		if ((dolby_vision_flags & FLAG_FORCE_DOVI_LL) || dolby_vision_ll_policy >= DOLBY_VISION_LL_YUV422) {
	...
	if ((dolby_vision_flags & FLAG_DISABLE_LOAD_VSVDB) == 0) {
		...
		if (vinfo &&  vinfo->vout_device &&
			...
			if (new_dovi_setting.vsvdb_changed && new_dovi_setting.vsvdb_len) {
				...
				pr_dolby_dbg("%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
	...
	if (dst_format == FORMAT_DOVI) {
		...
		if (dolby_vision_ll_policy == DOLBY_VISION_LL_RGB444 || (dolby_vision_flags & FLAG_FORCE_RGB_OUTPUT))
	...
	if (dst_format == FORMAT_HDR10 && (dolby_vision_flags & FLAG_DOVI2HDR10_NOMAPPING))
	...
	if (flag >= 0) {
		...
		if ((dolby_vision_flags & FLAG_FORCE_DOVI_LL) && dst_format == FORMAT_DOVI)
			new_dovi_setting.dovi_ll_enable = 1;
		if ((dolby_vision_flags & FLAG_FORCE_RGB_OUTPUT) && dst_format == FORMAT_DOVI) {
			new_dovi_setting.dovi_ll_enable = 1;
...
int dolby_vision_wait_metadata(struct vframe_s *vf)
{
	...
	if (dolby_vision_flags & FLAG_CERTIFICAION) {
		...
		if (setting_update_count > crc_count && !(dolby_vision_flags & FLAG_DISABLE_CRC)) {
			if (ott_mode)
				return 1;
		}
	}
	...
	if (is_dovi_dual_layer_frame(vf)) {
		...
		while (el_vf) {
			...
			if (el_vf->pts_us64 == vf->pts_us64 || !(dolby_vision_flags & FLAG_CHECK_ES_PTS)) {
				/* found el */
				ret = 3;
				break;
		...
		if (dolby_vision_policy_process(&mode, check_format)) {
			if (mode != DOLBY_VISION_OUTPUT_MODE_BYPASS &&
				...
				if (dolby_vision_policy == DOLBY_VISION_FOLLOW_SOURCE &&
					dolby_vision_mode == DOLBY_VISION_OUTPUT_MODE_BYPASS &&
					mode == DOLBY_VISION_OUTPUT_MODE_IPT_TUNNEL &&
					dolby_vision_wait_internal > 0 && !vf_is_fel(vf))
				{
					dolby_vision_wait_count = dolby_vision_wait_internal;
				} else {
					dolby_vision_wait_count = 0;
				}
...
int dolby_vision_process(struct vframe_s *vf, u32 display_size, u8 toggle_mode, u8 pps_state)
{
	...
	if ((dolby_vision_flags & FLAG_CERTIFICAION) &&
		...
		if (setting_update_count == 1 && crc_read_delay == 1) {
	...
	if ((!vf && video_turn_off) ||
		...
		if (dolby_vision_policy_process(&mode, FORMAT_SDR)) {
			...
			if (dolby_vision_policy == DOLBY_VISION_FOLLOW_SOURCE && mode == DOLBY_VISION_OUTPUT_MODE_BYPASS) {
				dolby_vision_target_mode = DOLBY_VISION_OUTPUT_MODE_BYPASS;
				dolby_vision_mode = DOLBY_VISION_OUTPUT_MODE_BYPASS;
		...
		if ((dolby_vision_flags & FLAG_TOGGLE_FRAME) || ((video_status == -1) && dolby_vision_core1_on)) {
	...
	if ((dolby_vision_flags & FLAG_CERTIFICAION) || (dolby_vision_flags & FLAG_BYPASS_VPP))
		video_effect_bypass(1);
	...
	if (dolby_vision_core1_on && dolby_vision_core1_on_cnt < DV_CORE1_RECONFIG_CNT && !(dolby_vision_flags & FLAG_TOGGLE_FRAME)) {
		dolby_vision_set_toggle_flag(1);
		if (!(dolby_vision_flags & FLAG_CERTIFICAION))
			pr_dolby_dbg("Need update core1 setting first %d times\n", dolby_vision_core1_on_cnt);
	}
	
	if (dolby_vision_on && !dolby_vision_core1_on && dolby_vision_core2_on_cnt && dolby_vision_core2_on_cnt < DV_CORE2_RECONFIG_CNT &&
		!(dolby_vision_flags & FLAG_TOGGLE_FRAME) && !(dolby_vision_flags & FLAG_CERTIFICAION)) {
	...
	if (dolby_vision_flags & FLAG_TOGGLE_FRAME) {
		...
		if (((new_dovi_setting.video_width & 0xffff) && (new_dovi_setting.video_height & 0xffff)) || force_set_lut) {
			...
			if (core1_disp_hsize != dovi_setting.video_width || core1_disp_vsize != dovi_setting.video_height)			
...
static s16 map_pq_inter_to_exter(enum pq_item_e pq_item, s16 inter_value)
{
	s16 inter_pq_min;
	s16 inter_pq_max;
	s16 exter_pq_min;
	...
	exter_value = exter_pq_min + (tmp - inter_pq_min) * exter_range / inter_range;

	if (debug_dolby & 0x200)
		pr_info("%s: %s [%d]->[%d]\n", __func__, pq_item_str[pq_item], inter_value, exter_value);
...
static s16 map_pq_exter_to_inter(enum pq_item_e pq_item, s16 exter_value)
{
	...
	if (debug_dolby & 0x200)
		pr_info("%s: %s [%d]->[%d]\n", __func__, pq_item_str[pq_item], exter_value, inter_value);
...
static s16 get_single_pq_value(int mode, enum pq_item_e item)
{
	...
	if (debug_dolby & 0x200)
		pr_info("%s: mode:%d, item:%s, [inter: %d, exter: %d]\n", __func__, mode, pq_item_str[item], inter_value, exter_value);
